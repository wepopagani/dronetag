...
export async function createInsurance(i: Omit<Insurance, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
  await delay();
  const id = makeId('ins');
  insurances.set(id, { ...i, id, createdAt: nowIso(), updatedAt: nowIso() });
  return id;
}
...
export async function createCertificate(c: Omit<Certificate, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
  await delay();
  const id = makeId('cert');
  certificates.set(id, { ...c, id, createdAt: nowIso(), updatedAt: nowIso() });
  return id;
}
